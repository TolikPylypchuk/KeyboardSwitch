...
pkgbuild --nopayload --identifier io.tolik.keyboardswitch.uninstaller --scripts scripts --version 4.1.0 \
...
KeyboardSwitchUninstaller-4.1.pkg

rm KeyboardSwitchUninstaller.pkg

xcrun notarytool submit KeyboardSwitchUninstaller-4.1.pkg --wait \
--apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$NOTARIZATION_PASSWORD"

xcrun stapler staple KeyboardSwitchUninstaller-4.1.pkg
